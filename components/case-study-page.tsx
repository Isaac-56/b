    ...
    >
      ...
      <section className="case-hero page-shell">
        ...
        <div className="case-hero__heading">
          <p className="kicker">
            {project.index} / {cleanText(project.eyebrow)}
          </p>

          <h1>{cleanText(project.headline)}</h1>
        </div>
      ...
      >
        ...
        <div className="device device--desktop case-showcase__device">
          ...
          <Image
            src={project.hero}
            alt={`${cleanText(project.title)} product interface`}
            fill
            priority
            sizes="90vw"
      ...
      <section className="case-story page-shell">
        <Reveal className="story-block">
          ...
          <p>{cleanText(project.challenge)}</p>
        </Reveal>
        ...
        <Reveal className="story-block" delay={80}>
          ...
          <p>{cleanText(project.approach)}</p>
        </Reveal>
      </section>
      ...
      <section className="case-gallery page-shell">
        <div className="section-heading">
          ...
          <h2>
            A product story told through its key moments.
          </h2>
        </div>
        ...
        <div className="case-gallery__grid">
          {project.gallery.map((item, index) => (
            <Reveal
              as="figure"
              className={`gallery-frame gallery-frame--${
                item.layout || "wide"
              }`}
              delay={(index % 2) * 80}
              key={item.src}
            >
              <Image
                ...
                sizes={
                  item.layout === "tall"
                    ? "(max-width: 800px) 94vw, 44vw"
                    : "(max-width: 800px) 94vw, 90vw"
                }
                style={{
      ...
      <section className="case-impact page-shell">
        <Reveal>
          ...
          <h2>{cleanText(project.impact)}</h2>

          <div className="impact-results">
            {project.results.map((result) => (
              <p key={result}>{cleanText(result)}</p>
            ))}
          </div>
      ...
      >
        ...
        <h2>{cleanText(nextProject.title)}</h2>

        <p>{cleanText(nextProject.headline)}</p>
